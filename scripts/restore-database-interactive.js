...
/**
 * Interactive Database Restoration Script for Alignzo
...
function runPsqlCommand(sql, description, username = DB_CONFIG.adminUsername, password = null) {
  const connectionString = password 
    ? `postgresql://${username}:${password}@${DB_CONFIG.host}:${DB_CONFIG.port}/${DB_CONFIG.database}`
    : `postgresql://${username}@${DB_CONFIG.host}:${DB_CONFIG.port}/${DB_CONFIG.database}`;
...
async function promptForPassword() {
  ...
  return new Promise((resolve) => {
    rl.question('🔐 Enter PostgreSQL admin password: ', (password) => {
      rl.close();
      resolve(password);
    });
  });
...
async function testAdminConnection() {
  ...
  DB_CONFIG.adminPassword = adminPassword;
  
  const adminConnectionString = `postgresql://${DB_CONFIG.adminUsername}:${adminPassword}@${DB_CONFIG.host}:${DB_CONFIG.port}/postgres`;
  ...
  try {
    ...
    log('Please check your PostgreSQL password and try again', 'yellow');
    process.exit(1);
  }
}
...
function createAppUser() {
  ...
  runPsqlCommand(sql, 'Creating application user and granting privileges', DB_CONFIG.adminUsername, DB_CONFIG.adminPassword);
}
...
function installExtensions() {
  ...
  runPsqlCommand(sql, 'Installing PostgreSQL extensions', DB_CONFIG.adminUsername, DB_CONFIG.adminPassword);
}
...
function showSuccess() {
  ...
  log(`Admin: postgresql://${DB_CONFIG.adminUsername}:***@${DB_CONFIG.host}:${DB_CONFIG.port}/${DB_CONFIG.database}`, 'yellow');
...
if (process.argv.includes('--help') || process.argv.includes('-h')) {
  console.log(`
Interactive Alignzo Database Restoration Script

Usage: node restore-database-interactive.js [options]

Options:
  --help     Show this help message

This script will:
1. Prompt for PostgreSQL admin password
2. Create the alignzo_v2 database
3. Create the alignzo user with proper permissions
4. Install required PostgreSQL extensions
5. Set up Prisma schema and generate client
6. Seed the database with initial data
7. Add sample work logs
...
Example:
  node restore-database-interactive.js
  `);
  process.exit(0);
}
